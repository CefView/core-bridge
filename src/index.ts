...
class CefViewBridge {
  ...
  constructor(bridgeName: string) {
    ...
    if (typeof window.cefViewQueryCancel !== "function") {
      throw new Error(
        `CefViewBridge: method window.cefViewQueryCancel was missing.`
      );
    }
  ...
  cancelQuery(queryId: number): void {
    window.cefViewQueryCancel(queryId);
  }
}
 ...
 * @returns
 */
function createBridge(name: string): CefViewBridge | undefined {
  try {
    return new CefViewBridge(name);
  } catch (error) {
    console.error("Failed to create CefViewBridge:", error);
    return undefined;
  }
}
